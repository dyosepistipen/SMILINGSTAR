...
class Cart extends Securearea {
  ...
  public function getcurrentcart($getValues = FALSE) {
    ...
    if (intval($this->carttotalitems) == 0) {
      if ($getValues == TRUE) {
        return "";
      }
      echo "";
      die;
    } //Kill it when there is no item in the cart
    //echo "calle";
    ...
    $retArr = array(
      ...
      "grossprice" => "Php. " . number_format(doubleval($totalprice) - doubleval($discount)),
      "totalprice" => "Php. " . number_format($totalprice),
      "totaldiscount" => "Php. " . number_format($discount),
    );
    if ($getValues == TRUE) {
      return $retArr;
    } else {
